...
description: Write content for a LinkedIn carousel, slide deck, or swipeable multi-slide format. Use when the user mentions "carousel," "slides," "LinkedIn carousel," "swipe post," "slide deck," or "visual content" and wants the slide-by-slide text copy drafted. Outputs slide-by-slide text content (not visual design). Pairs with the threads-carousel skill to render the text into images.
---
...
If a `workspace/social/[C] social-context.md` file exists, read it to understand the user's voice, tone, content pillars, and platform preferences. Match vocabulary, sentence structure, and emotional register.

If the file does not exist, note that a voice-context file would make the copy sound more like the user, but do not block — proceed with a neutral, clear voice.
...
- To render the written slides into actual carousel images, hand the slide blocks to the **threads-carousel** skill (Next.js engine + PNG/PDF export)
- Does not analyze post performance or metrics
- Does not handle scheduling or calendar planning