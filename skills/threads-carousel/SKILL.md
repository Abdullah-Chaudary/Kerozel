...
description: >
  ...
  Use when the user wants the actual rendered carousel images, PNG/PDF export, or a live preview after slide text has been written.
  Pair with the social-carousel-writer skill for high-quality slide copy, then render the results with this engine.
...
In opencode, the skill is triggered by simply asking for a carousel ("make a Threads carousel from this text", "render these slides", etc.). Recommended flow:

1. **Write first** — use the `social-carousel-writer` skill to draft high-quality slide-by-slide copy (cover / context / body / CTA).
2. **Render** — map that copy into `src/slides.ts` (see Slide types below) and launch the preview server with this skill.
3. **Export** — user downloads PNG(s) or a single PDF from the browser toolbar.

## Working location (Windows / opencode)

A dedicated persistent working copy lives at `~/carousel-studio/` (a one-time copy of the template engine). Every job edits only `~/carousel-studio/src/slides.ts` — never the engine files (`src/app/CarouselApp.tsx`, `src/lib/*`). This avoids the per-run `rsync`/symlink workflow that the original Claude Code skill used.
...
1. User drops a file path (or passes a file via the chat).
2. Copy it into `~/carousel-studio/public/images/` with a safe, lowercase filename — e.g. `Copy-Item -LiteralPath "$USER_PATH" "$env:USERPROFILE\carousel-studio\public\images\screenshot.png"`.
3. In `~/carousel-studio/src/slides.ts` reference it as `imageSrc: "/images/screenshot.png"` (absolute from `/public/`).
...
The engine already lives in `~/carousel-studio/`. If it does not exist yet, create it once:

```powershell
# One-time setup (already done during installation)
Copy-Item -Recurse -Force "$env:USERPROFILE\.config\opencode\skills\threads-carousel\template" "$env:USERPROFILE\carousel-studio"
# then: cd ~/carousel-studio && bun install
```

For every job: work directly in `~/carousel-studio/`. Nothing to copy per run.

#### Inject content into `src/slides.ts`

All content + defaults live in `~/carousel-studio/src/slides.ts` — never touch the engine. Edit the `SLIDES` array and the 5 default constants.
...
Run the dev server in the background (or a separate terminal) and tell the user to open `http://localhost:3333`. They can:
...
For multiple carousels at once: clone the studio folder (e.g. `~/carousel-studio-2/`) and launch on ports 3333, 3334, 3335 — they run side by side without conflict. Each studio copy gets its own `src/slides.ts`.
